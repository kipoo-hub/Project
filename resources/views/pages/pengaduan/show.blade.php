                    ...
                    <div class="card-body p-4">
                        ...
                        @if ($pengaduan->lampiran)
                            ...
                            <div class="mb-4">
                                ...
                                @if (!empty($pengaduan->lampiran) && Str::endsWith($pengaduan->lampiran, ['jpg', 'jpeg', 'png']))
                                    {{-- Jika berupa gambar --}}
                                    ...
                                    <div>
                                        <a href="{{ asset('storage/' . $pengaduan->lampiran) }}" target="_blank"
                                            ...
                                            <i class="fas fa-image me-1"></i> Buka Gambar
                                        </a>
                                    </div>
                                @elseif (!empty($pengaduan->lampiran))
                                    {{-- Jika berupa PDF --}}
                                    <div class="d-flex align-items-center gap-2">
                                        <i class="fas fa-file-pdf fa-3x text-danger"></i>
                                        <a href="{{ asset('storage/' . $pengaduan->lampiran) }}" target="_blank"
                                            class="btn btn-outline-primary btn-sm">
                                            <i class="fas fa-paperclip me-1"></i> Buka Dokumen
                                        </a>
                                    </div>
                                @else
                                    {{-- Jika TIDAK ADA lampiran → Placeholder --}}
                                    <img src="{{ asset('assets/img/placeholder.png') }}"
                                        class="img-fluid rounded shadow-sm" style="max-height: 300px; object-fit: cover;">
                                    <p class="text-muted small mt-2">Tidak ada lampiran yang diunggah.</p>
                                @endif
                            </div>
                        @endif
                            ...
                            <div class="mt-4">
                                ...
                                @forelse($pengaduan->tindakLanjuts as $tindak)
                                    <div class="alert alert-success">
                                        <h5 class="alert-heading">Jawaban Petugas
                                            ({{ $tindak->petugas->name ?? 'Petugas' }})</h5>
                                        <p>{{ $tindak->isi_tindak_lanjut }}</p>
                                        <hr>
                                        <p class="mb-0 small text-muted">{{ $tindak->created_at->format('d M Y, H:i') }}
                                        </p>
                                    </div>
                                @empty
